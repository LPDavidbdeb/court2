...
class Command(BaseCommand):
    help = 'Adds new photos from a source directory, processing both loose files and files in date-named subdirectories.'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'--- Starting to add new photos from {source_directory} ---'))
        ...
        for item_name in sorted(os.listdir(source_directory)):
            ...
            if os.path.isfile(item_path):
                if item_name.lower().endswith(service.supported_extensions):
                    if Photo.objects.filter(file_path=item_path).exists():
                        skipped_count += 1
                        continue
        ...
        self.stdout.write(f'{skipped_count} photos were skipped (already in database or no date found).')
